...
import { Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, Input, Checkbox, ButtonGroup } from '@nextui-org/react';
...
export default function EmailsPage() {
  ...
  const [emailValueUp, setEmailValueUp] = useState('');
  const [usernameValueUp, setUsernameValueUp] = useState('');
  const [passwordValueUp, setPasswordValueUp] = useState('');
  ...
  const [selectedEmail, setSelectedEmail] = useState(null); // хранит выбранную почту

  const handleSelectEmailTab = (email) => {
    setSelectedTab('selected-email'); // переключаем таб на выбранную почту
    setSelectedEmail(email); // сохраняем выбранную почту в состояние
  };
  ...
  const { register, handleSubmit, formState: { errors, touchedFields, isValid, isSubmitting }, setValue, trigger, clearErrors } = useForm({
      mode: 'onChange', // Валидация при изменении значений
      resolver: yupResolver(schema),
  });

  const { update, handleUpdate, formState: { errorsUp, touchedFieldsUp, isValidUp, isSubmittingUp }, setValueUp, triggerUp, clearErrorsUp } = useForm({
    mode: 'onChange', // Валидация при изменении значений
  ...
  const handleInputChangeUpdate = (e, field) => {
    let value = e.target.value;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        fetchEmails(); // Обновляем список почт
        setUsernameValue(''); // Сбрасываем поля
        setPasswordValue('');
        setEmailValue('');
      }
    } catch (error) {
  ...
  const [loadingStates, setLoadingStates] = useState({}); // Состояние для каждого email
  
  // Функция для показа/скрытия пароля с учетом загрузки
  const handleShowPassword = async (emailId) => {
    setLoadingStates((prevState) => ({
      ...prevState,
      [emailId]: true, // Устанавливаем состояние загрузки только для текущего email
    }));
    ...
    if (showPassword[emailId]) {
      ...
      setLoadingStates((prevState) => ({
        ...prevState,
        [emailId]: false, // Остановка загрузки при скрытии пароля
      }));
    } else {
      try {
        ...
        setLoadingStates((prevState) => ({
          ...prevState,
          [emailId]: false, // Остановка загрузки после запроса
        }));
      }
    }
  };
  ...
  const handleSelectAll = () => {
    if (isAllChecked) {
      setSelectedEmails([]); // Если уже выбраны все, снимаем выделение
      setIsAllChecked(false);
      setIsIndeterminate(false);
    } else {
      const allEmailIds = emails.map((email) => email._id);
      setSelectedEmails(allEmailIds); // Выделяем все почты
      setIsAllChecked(true);
      setIsIndeterminate(false);
    }
  };

  // Функция для изменения состояния чекбокса конкретной почты
  ...
  useEffect(() => {
    if (selectedEmails.length === 0) {
      ...
      setIsIndeterminate(true); // Устанавливаем isIndeterminate, если выбраны не все почты
    }
  ...
  useEffect(() => {
    if (selectedEmail) {
      handleShowPassword(selectedEmail._id); // Fetch the decrypted password when the form loads
    }
  }, [selectedEmail]);
  ...
  const onUpdate = async (data) => {
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/emails/${selectedEmail._id}`, payload);
      ...
      if (response.status === 200) {
        ...
        fetchEmails(); // Обновление списка почт после изменения
      }
    } catch (error) {
    ...
    <div className="w-full h-full">
        <div className="max-w-1320 py-8 flex flex-col  mx-auto">
            ...
            <div id="navigation-mail" className="border border-[#E9EBF3] rounded-t-lg flex">
              {/* Существующие кнопки для переключения табов */}
              <button
              ...
              {selectedEmail && (
                <button
                  ...
                  className={`emails-nav flex items-center justify-center px-8 py-3 ${selectedTab === 'selected-email' ? 'active' : ''}`}
                >
              ...
              >
                {selectedTab === 'allMails' ? (
                    ...
                    <table className="table-auto w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-[#E9EBF3] text-sm text-[#343F52] font-medium">
                          ...
                          <th className="px-6 py-3 w-1/4">Пользователь</th>
                          <th className="px-6 py-3 w-1/3">Пароль</th>
                          <th className="px-6 py-3 w-1/4"></th>
                          <th className="px-6 py-3 w-1/6">Действия</th>
                        </tr>
                      </thead>
                      <tbody>
                        {emails.map((email) => (
                          <tr key={email._id} className="border-b font-medium text-[#343F52]">
                            ...
                            <td className="px-6 py-3">
                              <div className="relative w-full flex gap-2">
                                <input
                                  ...
                                  disabled
                                  className="text-sm border w-full rounded-lg px-3 py-1.5 text-[#343F52] outline-none duration-150"
                                />
                                  {isAdmin && isTwofaEnabled && (
                                    <Button
                                      isDisabled={!isAdmin && !isTwofaEnabled}
                                      onClick={() => handleShowPassword(email._id)}
                                      isLoading={loadingStates[email._id]} // Загрузка только для конкретного email
                                      className="bg-transparent border hover:bg-[#e5e7eb]"
                                    >
                                      <img
                                        src={showPassword[email._id] ? '/assets/img/mdi_eye_off.svg' : '/assets/img/mdi_eye.svg'}
                                        alt="toggle-password"
                                        className={`w-4 h-4 ${loadingStates[email._id] ? 'hidden' : ''}`} // Скрываем иконку при загрузке
                                      />
                                    </Button>
                                  )}
                              </div>  
                            ...
                            <td className="px-6 py-3 ">
                              <div className="flex justify-between items-center ">
                                <ButtonGroup>
                                    <Button className="*:fill-[#343F52] hover:bg-[#e5e7eb] bg-transparent border" onClick={() => handleSelectEmailTab(email)}> 
                                    ...
                                    <Button isDisabled={!isAdmin && !isTwofaEnabled} onClick={() => openDeleteModal(email)} className=" border border-l-0 bg-transparent hover:bg-[#e5e7eb] *:fill-[#343F52] "> 
                    ...
                    <div>
                      ...
                      <form onSubmit={handleSubmit(onSubmit)}>
                          ...
                          <Button
                            ...
                            type="submit" // Связываем с отправкой формы
                          >
                            Сохранить почту
                          </Button>
                        </form>
                  ...
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                  >
                    {/* Form for editing selected email */}
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex flex-col mb-4 w-full">
                          ...
                          <div className="flex w-full">
                            <input
                              type="text"
                              value={selectedEmail.email.split('@')[0]} // Показываем часть email до @
